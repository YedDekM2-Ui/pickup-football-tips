...
/* ---------- เวลาไทยบนการ์ด — ต้องอ่านแล้วรู้เลย ไม่ต้องแปลเอง ---------- */

/** สร้างมิลลิวินาทีจากเวลาท้องถิ่น — ตัวปลอม formatDate ใน gasEnv ไม่สนใจ tz
    ถ้าไปสร้างด้วย Date.UTC เทสต์จะเพี้ยนตามเครื่องที่รัน */
function ms(y, mo, d, h, mi) { return new Date(y, mo - 1, d, h, mi, 0, 0).getTime(); }

test('fsSpanTh_ พูดช่วงเวลาเป็นภาษาคน', () => {
  const g = env({ FB_PROXY: '-' });
  eq(g.fsSpanTh_(45), '45 นาที');
  eq(g.fsSpanTh_(60), '1 ชม.');
  eq(g.fsSpanTh_(120), '2 ชม.');
  eq(g.fsSpanTh_(135), '2 ชม. 15 นาที');
  eq(g.fsSpanTh_(0), '0 นาที');
});

test('fsKoThai_ วันนี้ = บอกวันนี้ + เหลืออีกเท่าไหร่', () => {
  const g = env({ FB_PROXY: '-' });
  eq(g.fsKoThai_(ms(2026, 9, 4, 18, 0), ms(2026, 9, 4, 16, 0)),
     'วันนี้ 18:00 น. (อีก 2 ชม.)');
});

test('fsKoThai_ เตะไปแล้ว = บอกว่าไปแล้วกี่นาที พร้อมเวลาจริงในวงเล็บ', () => {
  const g = env({ FB_PROXY: '-' });
  eq(g.fsKoThai_(ms(2026, 9, 4, 15, 20), ms(2026, 9, 4, 16, 0)),
     'เตะไปแล้ว 40 นาที (15:20 น.)');
});

test('fsKoThai_ ข้ามคืน = พรุ่งนี้ ไม่ใช่ 09-05', () => {
  const g = env({ FB_PROXY: '-' });
  eq(g.fsKoThai_(ms(2026, 9, 5, 1, 45), ms(2026, 9, 4, 22, 0)),
     'พรุ่งนี้ 01:45 น. (อีก 3 ชม. 45 นาที)');
});

test('fsKoThai_ เกิน 24 ชม. = ตัดหางอีกกี่ชั่วโมงทิ้ง (บอกไปก็ไม่ช่วย)', () => {
  const g = env({ FB_PROXY: '-' });
  /* พรุ่งนี้แต่ไกลเกินวัน — คำว่าพรุ่งนี้ยังอยู่ หางหายไป */
  eq(g.fsKoThai_(ms(2026, 9, 5, 13, 0), ms(2026, 9, 4, 12, 0)), 'พรุ่งนี้ 13:00 น.');
  /* ไกลกว่านั้น = วันที่ + เดือนไทย */
  eq(g.fsKoThai_(ms(2026, 9, 7, 20, 0), ms(2026, 9, 4, 12, 0)), '7 ก.ย. 20:00 น.');
});

test('fsKoThai_ ไม่มีเวลาเตะ = เงียบ ไม่ใช่ 1 ม.ค. 2513', () => {
  const g = env({ FB_PROXY: '-' });
  eq(g.fsKoThai_(0), '');
  eq(g.fsKoThai_(null), '');
});

test('fsNowThai_ หัวการ์ดเป็นวันที่ไทย ไม่ใช่ 03/09', () => {
  const g = env({ FB_PROXY: '-' });
  const s = g.fsNowThai_();
  ok(/^\d{1,2} [\u0e00-\u0e7f.]+ \d{2}:\d{2} น\.$/.test(s), s);
});
