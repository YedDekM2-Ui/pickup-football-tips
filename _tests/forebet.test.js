...
test('แถวเก่าที่ติดตัวย่อ Co1 ต้องซ่อมตัวเองได้ — แก้ช่องลีกช่องเดียว ไม่เพิ่มแถว', () => {
  ...
  ok(r.fixed.join(' · ').indexOf('Primera A') >= 0, 'รายงานต้องบอกว่าซ่อมอะไรไป');
  ...
  eq(row['เดาสกอร์'], '1-1', 'ภาพนิ่งคำทำนายของเดิมต้องอยู่ครบ');
  /* เวลาเตะไม่ใช่คำทำนาย มันคือ "ข้อเท็จจริง" — ของเดิมผิดต้องซ่อมให้ตรงหน้าเว็บ */
  eq(row['เวลาเตะ'], '10:00', 'เวลาเตะซ่อมตามหน้าเว็บรอบล่าสุด');
  ok(r.fixed.join(' · ').indexOf('วันเวลา=') >= 0, 'รายงานต้องบอกว่าซ่อมวันเวลาไปด้วย');
});
...
test('เวลาไทย: วัน/เดือน สลับกันได้ตามคนขอ ต้องเลือกแบบที่เป็นวันจริงได้', () => {
  const g = fbNet(bookOf([]), () => fakeResponse(200, pageHtml(PAGE_A)));
  /* เขาโชว์แบบเดือน/วัน + AM/PM (แบบที่ Jina เคยเห็น) — 26 เป็นเดือนไม่ได้ ต้องอ่านเป็นวัน */
  const a = g.fbWhenLocal_('2026-08-26', '08/26/2026 3:30 PM');
  eq(a.date, '2026-08-26'); eq(a.time, '22:30');
  /* เขาโชว์แบบวัน/เดือน 24 ชม. — ตัวเลขนำหน้าคือ 25 ไม่ใช่เดือน */
  const b = g.fbWhenLocal_('2026-08-25', '25/08/2026 18:30');
  eq(b.date, '2026-08-26', 'บวก 7 แล้วข้ามเที่ยงคืน'); eq(b.time, '01:30');
  /* กำกวมทั้งคู่ (05/08) = ให้ startDate ชี้ขาด */
  eq(g.fbWhenLocal_('2026-05-08', '05/08/2026 10:00', 0).date, '2026-05-08', 'ISO บอกว่าเดือน 5');
  eq(g.fbWhenLocal_('2026-08-05', '05/08/2026 10:00', 0).date, '2026-08-05', 'ISO บอกว่าเดือน 8');
  eq(g.fbWhenLocal_('', '05/08/2026 10:00', 0).date, '2026-08-05', 'ไม่มี ISO = เอา วัน/เดือน แบบยุโรป');
});

test('เวลาไทย: วันต้องมาจากข้อความก้อนเดียวกับเวลา ห้ามให้ startDate ของคู่อื่นลากวันไป 1 วัน', () => {
  const g = fbNet(bookOf([]), () => fakeResponse(200, pageHtml(PAGE_A)));
  /* ของจริง 26 ส.ค. 69: หน้าเว็บเขียน "26/08/2026 17:30" แต่ startDate ในก้อนเป็นของคู่ถัดไป (27)
     ที่ถูกคือ 26/8 22:30 ตามที่เจ้าของทักมา ไม่ใช่ 27/8 */
  const a = g.fbWhenLocal_('2026-08-27', '26/08/2026 17:30', 5);
  eq(a.date, '2026-08-26', 'ต้องเป็นวันที่ 26 ตามข้อความ');
  eq(a.time, '22:30');
});
...
test('เวลาไทย: อ่านเวลาไม่ออก = ปล่อยว่าง ห้ามเดา', () => {
  ...
  eq(g.fbWhenLocal_('', '18:30'), null, 'ไม่มีวันเลยสักทาง = ไม่คำนวณ');
  eq(g.fbWhenLocal_('', '25/08/2026 18:30').date, '2026-08-26', 'ไม่มี ISO แต่ข้อความมีวัน = ใช้ได้');
...
test('คู่ที่เตะจบแล้วต้องหลุดตรงเวลา — เวลาในชีตคือเวลาไทย ลบ 7 เสมอ', () => {
  /* กันเลขปนกัน: FB_TZ_SHIFT ("เวลาที่เขาโชว์ -> เวลาไทย" ของจริง = 5) ห้ามหลุดมาโผล่
     ในตัวคิดเวลาเตะ ถ้าหลุดมาเมื่อไหร่ คู่ที่จบไปแล้วจะค้างหน้าเว็บต่ออีก 2 ชม. */
  const g = fbEnvP(bookOf([]), '');
  g.PropertiesService.getScriptProperties().setProperty('FB_TZ_SHIFT', '5');
  eq(g.prop_('FB_TZ_SHIFT'), '5', 'ต้องตั้งค่าติดจริง ไม่งั้นเทสต์ผ่านแบบไม่ได้ทดสอบอะไร');
  const rows = [{ 'ทีมเหย้า':'A', 'ทีมเยือน':'B', 'วันที่':'2026-08-26', 'เวลาเตะ':'22:30' }];
  eq(g.fbUpcoming_(rows, T('2026-08-26T22:29:00+07:00')).length, 1, 'ก่อนเตะ 1 นาที ต้องยังอยู่');
  eq(g.fbUpcoming_(rows, T('2026-08-26T22:31:00+07:00')).length, 0, 'เตะไปแล้วต้องหลุด');
});
