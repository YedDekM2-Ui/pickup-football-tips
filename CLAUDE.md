...
- โค้ดครบงาน 1–9 · เทสต์ `ผ่าน 108/108`
...
- **คู่ปักหมุดหน้า 1** (`gas/Forebet.gs`): ดึง Featured match / Pick of the day จาก forebet เก็บเป็น **ภาพนิ่ง** ลงแท็บ PICKS (ช่อง `FEATURED` / `POTD`)
  ...
  - ทางเข้า (หลังด่านกุญแจทั้งคู่): `?p=snap` = ดึง 1 รอบ + ติดตั้ง trigger 6 ชม. · `?p=fbprobe` = ส่องว่าแกะหน้าเว็บออกไหม
- **ค้างที่เจ้าของ**: เปิด `?p=snap` 1 ครั้ง (ติด trigger) · ส่งผล `?p=fbprobe` กลับมาถ้าคู่ปักหมุดไม่ขึ้น · BotFather / rotate คีย์ Vision
