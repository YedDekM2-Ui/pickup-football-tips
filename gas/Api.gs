...
function doGet(e) {
  ...
  if (p === 'hook') return jsonOut_(tgSetHook_(String(q.url || '')));
    ...
    /* ตัวถามผลหวยต้องอยู่ "นอก" tgPoll_ — ข้างในมันคืนตั้งแต่บรรทัดแรกถ้าโหมดไม่ใช่ poll
       ของเดิมหวยเลยเงียบตามไปด้วยทั้งที่คนละเรื่องกัน */
    if (p === 'poll') {
      var pollRes = tgPoll_();
      try { lotAutoAsk_(); } catch (err) { /* หวยพังห้ามลามไปทำให้ดึงข้อความพัง */ }
      return jsonOut_(pollRes);
    }
    ...
    /* ชื่อทางที่ไม่รู้จักต้องฟ้อง ห้ามเงียบ ๆ คายก้อนหน้าเว็บกลับไป
       ของเดิมพิมพ์ผิดแล้วได้ payload ก้อนเดิม เลยดูเหมือนคำสั่งทำงานสำเร็จ */
    if (p !== 'all') return jsonOut_({ ok: false, error: 'ไม่รู้จักคำสั่ง: ' + p });
